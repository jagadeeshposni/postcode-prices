...
export async function fetchSampleData(query: string) {
  ...
  try {
    ...
    ROUND(AVG(CASE WHEN property_type = 'S' THEN price ELSE NULL END))::INTEGER AS SemiDetached,
    ROUND(AVG(CASE WHEN property_type = 'D' THEN price ELSE NULL END))::INTEGER AS Detached,
    ROUND(AVG(CASE WHEN property_type = 'F' THEN price ELSE NULL END))::INTEGER AS Flats
    FROM 
        sample
    WHERE
        postcode like $1 || '%'
